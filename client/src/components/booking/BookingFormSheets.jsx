...
export default function BookingFormSheets({ rooms, selectedDate, selectedRoom, bookings = [], onBook, onClose }) {
  ...
  const bookedSlots = useMemo(() => {
    ...
    bookings.filter(b => b.room === form.room && b.status !== 'cancelled').forEach(b => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await onBook({
        name: form.name.trim(), phone: form.phone.trim(), dept: form.dept.trim(),
        room: form.room, date: selectedDate,
  ...
  const canSubmit = form.room && form.startTime && form.endTime && dur > 0;
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-fade-in"
      ...
      <div className="bg-white rounded-3xl w-full max-w-[600px] max-h-[93vh] overflow-y-auto animate-slide-up"
        ...
        <div className="px-6 pt-5 pb-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10 rounded-t-3xl">
          <div>
            <div className="font-bold text-lg text-gray-800" style={{ fontFamily: 'Prompt, sans-serif' }}>📝 จองห้องประชุม</div>
            <div className="text-xs text-gray-400 mt-0.5" style={{ fontFamily: 'Sarabun, sans-serif' }}>{formatDateTH(selectedDate)} · รอการอนุมัติจากเจ้าหน้าที่</div>
          </div>
          ...
          {step === 2 && (
            <div className="animate-fade-in space-y-4">
              <div>
                ...
                <select className="form-control cursor-pointer" value={form.room} onChange={e => { set('room', e.target.value); set('startTime', ''); set('endTime', ''); }} style={{ fontFamily: 'Sarabun, sans-serif' }}>
                  <option value="">-- เลือกห้องประชุม --</option>
                  {rooms.filter(r => r.isActive !== false).map(r => (
                    <option key={r.id} value={r.name}>{r.icon} {r.name} (ความจุ {r.capacity} คน · ชั้น {r.floor})</option>
                  ))}
                </select>
                <Err msg={errors.room} />
              </div>
              ...
              {canSubmit && (
                <div className="rounded-xl p-3 border border-teal-200 text-xs" style={{ background: '#f0fdf9', fontFamily: 'Sarabun, sans-serif' }}>
                  <span className="font-bold text-teal-700">{roomObj?.icon} {form.room}</span>
              ...
              {canSubmit && (
                <div className="rounded-xl p-4 border border-teal-200 space-y-1.5 text-xs" style={{ background: '#f0fdf9', fontFamily: 'Sarabun, sans-serif' }}>
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                    ...
                    <span className="font-semibold text-gray-800">{roomObj?.icon} {form.room}</span>
                  ...
                  {files.length > 0 && <div className="text-teal-600">📎 แนบ {files.length} ไฟล์</div>}
                  <div className="text-amber-600 text-[11px] pt-1">⏳ การจองจะมีผลหลังจากเจ้าหน้าที่อนุมัติ</div>
                </div>
              )}
            </div>
          )}
